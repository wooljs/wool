...
exports.inject = function (logger, mime) {
	var injected = {};

	injected.data = function(code, text) {
		return function (res, path, data) {
			logger.trace('data(code:{}, path:{}, data:{})', code, path, data);
			var type = mime(path);
			logger.trace('{} mime type {}', path, type);
	...
	injected.created = function(code,text) {
		return function (res, url) {
			logger.trace('created(code:{}, text:{}, url:{})', code, text, url);
	...
	injected.no_response = function(code,text) {
		return function (res) {
			logger.trace('no_response(code:{}, text:{})', code, text);
			res.writeHead(code);
			res.end();
		}
	}
	...
	injected.error = function(code, text) {
		return function (res, provide) {
			logger.trace('error(code:{}, text:{})', code, text);

		...
		build: function (biz, login_url, handler) {
			return function (req, res) {
				...
				if (req.headers.hasOwnProperty(TOKEN_HEADER)) {
					...
					if (biz.valid(session_id) && biz.auth(session_id, parsed.pathname)) { handler(req, res); }
				...
				if (req.method=='POST' && req.url==login_url) {
					...
					req.on('data', function (data) {
						try {
							obj = JSON.parse(data);
							biz.login(obj,
								function(session_id) { http_status(201)(res,'application/json','"'+session_id+'"'); },
								function() { http_status(401)(res); }
							);
						} catch(e) {console.dir(e); obj=false; }
					});

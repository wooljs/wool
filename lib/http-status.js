...
exports.inject = function () {
	...
	injected.created = function(code,text) {
		return function (res, type, val, header) {
			console.log(header);
			header = header || {}
			if (! ('Content-Type' in header)) header['Content-Type'] = type;
	...
	injected.error = function(code, text) {
		return function (res, provide, header) {
			provide = provide || injected.default_provide;
			header = header || {}
			if (! ('Content-Type' in header)) header['Content-Type'] = 'text/html';
			res.writeHead(code, header);
...
//	define(203, "Partial Information", );
	...
	define(409, "Conflict", injected.error);

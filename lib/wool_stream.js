...
module.exports = function (util, stream) {
    ...
    function StreamSplit(sep) { 
        if (!(this instanceof StreamSplit)) return new StreamSplit()
        this.lastLine = ''
    ...
    StreamSplit.prototype._transform = function (data, encoding, callback) {
        var p, c; p = c = 0
        while (c < data.length) {
            while (c < data.length && data[c] !== this.sep) { c+=1 }
            if (data[c] === this.sep) {
    ...
    export.StreamSplit = StreamSplit
    ...
    export.JsonStringify = JsonStringify
    ...
    export.StreamDispatch = StreamDispatch

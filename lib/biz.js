...
exports.inject = function (db, moment) {
	return {
		build: function (validator, mapping) {
			...
			var internal = function (t) {
				...
				if (typeof h === 'undefined') {
					...
					var val = validator(t)
					h = {
						getById: function(id,cb) {col.find({_id: db.objectId(id)},function (err, docs) { docs = docs || []; cb(err,docs[0])})},
				...
				auth: function (sid, path, cb) {
					if (sid) {
						internal('session').getById(sid,function(err,doc) {
							if (!err && doc && (moment().valueOf() < moment(doc.expirationDate).valueOf()))
								internal('user').getById(doc.user.toString(), function(err,user) {
									if (!err) validator('user').isAuthorized(user, path, internal('role'), cb)
									else cb(false)
								})
							else cb(false) 
						})

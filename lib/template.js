...
module.exports = function (http_status, fs, urlparser, str_to_obj, mu2) {
	var injected={};
	injected.regex = /^\/?(.*?)(-(?:.*?))?(?:\.html)?$/
	injected.build = function (template_base_folder,data_base_folder, regex, str_to_obj_inject) {
		template_base_folder = template_base_folder || '.'
		data_base_folder = data_base_folder || template_base_folder
		regex = regex || this.regex
		str_to_obj_inject = str_to_obj_inject || function() {}
		...
		return function (req, res) {
			...
			parsed.pathname.replace(regex,function (_0, _1, _2) {
				var template_name = _1 || 'index',
					...
					data_path =  data_folder + '/' + (template_name + _2 || template_name) + '.json'
				
				mu2.compile(template_path, function (err, template) {
					function render(data) {
						var stream = mu2.render(template, data)
						var html = ''
						...
						fs.stat(data_folder, function (err, stats) {
							if (err && err.errno==34) {
								...
								fs.readFile(data_path, function (err, data) {
									if (err) {
										manage_error(err,data_path,res,'Data: ')
									} else {
										console.error(Date(), "Data found", data_path)
										try {
											var d = str_to_obj(String(data),str_to_obj_inject(req))
											console.log(d)
										} catch (e) {
											console.error(Date(), "error while treating data:", e)
											http_status(500)(res)
											return
										}
										render(d)

...
module.exports = function (http_status, fs, urlparser, str_to_obj, mu2, knit) {
	var injected={};
	injected.regex = /^\/?(.*?)(-(?:.*?))?(?:\.html)?$/
		...
		return function (req, res) {
			...
			parsed.pathname.replace(regex,function (_0, _1, _2) {
				var template_name = _1 || 'index',
					...
					data_path =  data_folder + '/' + template_name+ ( !!_2 ? _2 : '') + '.json'
							...
							if (err && err.errno==34) {
								...
								fs.readFile(data_path, function (err, data) {
									if (err) {
										...
										try {
											var d = str_to_obj(String(data),str_to_obj_inject(req,knit))
										} catch (e) {

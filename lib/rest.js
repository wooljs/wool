					...
					if (typeof entity_type != 'undefined') {
						var sub = {
							get: function (parsed, req, res) {
								...
								if (_2.length > 0) { biz.on(entity_type).getById(_2, ok_200); }
							...
							post: function(parsed, req, res) {
								...
								req.on('data', function (data) {
									obj=JSON.parse(data);
									biz.on(entity_type).save(obj, function(id) {
										if (typeof data != 'undefined') { http_status(201)(res, type, '"'+id+'"');}
										else { http_status(403)(res); }
									});
								});
							...
							delete: function (parsed, req, res) {
								// This URI contains an id
								if (_2.length > 0) { biz.on(entity_type).remove(_2, function() {http_status(204)(res);}); }
								// Retrieve all objects that matches the query 
								// else { biz.on(entity_type).find(parsed.query, ok_200); }
							}	
						}
